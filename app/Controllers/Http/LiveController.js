...
const url = use('url');
const { http_get, http_post, md5, timestamp, is_string, is_empty } = require('../../Tools/helper');

class LiveController {
  async douyu({ request, response }) {
    const roomid = request.input('roomid', 1863767);
    const { data } = await http_get('https://web.sinsyth.com/lxapi/douyujx.x', {roomid: roomid});
  ...
  async huya({ request, response }) {
    ...
    const { data } = await http_get(`https://m.huya.com/${roomid}`, {}, {
  ...
  async huomao({ request, response }) {
    ...
    const { data } = await http_get(`https://www.huomao.com/mobile/mob_live/${roomid}`, {});
    ...
    const res_data = await http_post('https://www.huomao.com/swf/live_data', params);
  ...
  async huajiao({ request, response }) {
    ...
    const { data } = await http_get(`https://h.huajiao.com/api/getFeedInfo?sid=${timestamp()}&liveid=${roomid}`, {});
  ...
  async zhanqi({ request, response }) {
    ...
    const { data } = await http_get(`https://m.zhanqi.tv/api/static/v2.1/room/domain/${roomid}.json`, {});
  ...
  async yy({ request, response }) {
    ...
    const { data } = await http_get(`http://interface.yy.com/hls/new/get/${roomid}/${roomid}/1200?source=wapyy&callback=jsonp3`);
    if(data.code !== 0) {
      return response.json({code: 500, message: 'no'});
    }
  ...
  async bilibili({ request, response }) {
    ...
    const { data } = await http_get(`https://api.live.bilibili.com/xlive/web-room/v1/index/getRoomPlayInfo?room_id=${roomid}&play_url=1&mask=1&qn=4&platform=${platform}`, {}, {
